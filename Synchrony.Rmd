...
mod <- brm(pearson_r~0 + intercept + Distance + (1|int_group), cores=4, data = sync_data)

h <- "sd_int_group__Intercept^2 / (sd_int_group__Intercept^2 + sigma^2) = 0"
temp <- hypothesis(mod, h, class=NULL)


mod <- brm(pearson_r~Distance + (Distance|spp_pair), cores=4, data = sync_data)
...
mod2 <- brm(pearson_r~Distance + (1|spp_pair + Region/hilo), cores=4, data = sync_data)
```
