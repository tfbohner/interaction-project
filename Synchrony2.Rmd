...
summary(lm(.value~total_ppt_mm_fn1*pair, data=site_sp_summ))
...
#          .upper=ifelse(pair1_comp=="intra", .upper, .upper*-1),
#          .lower=ifelse(pair1_comp=="intra", .lower, .lower*-1)) %>% 
#   ggplot(aes(y = new_pair, x = spp_mean, color=site)) +
#   geom_pointintervalh(position = position_dodgev(height=0.5)) +
#   geom_vline(xintercept=0, linetype="dashed") +
#   theme_test() + ylab("Species pair comparisons") + xlab("Synchrony contrast estimate") +
...
a <- ggplot(precip_summary_raw, aes(decade, spei12_mean, color=Region)) +
  geom_point() +
  stat_summary(fun.y=mean, aes(group=interaction(Region, hilo), linetype=hilo), geom="line") +
  theme_test() + theme(legend.position = 'none')

b <- ggplot(precip_summary_raw, aes(decade, spei12_sd, color=Region)) +
  geom_point()  +
  stat_summary(fun.y=mean, aes(group=interaction(Region, hilo), linetype=hilo), geom="line") +
  theme_test() + theme(legend.position = 'none')
...
plot_grid(s1,
          plot_grid(s2, s3, align='v', axis='lr', rel_heights = c(0.3,1), ncol=1, labels=c('b','c')), labels='a')
...
plot_grid(s1b,
          plot_grid(s2, s3b, align='v', axis='lr', rel_heights = c(0.3,1), ncol=1, labels=c('b','c')), labels='a')
...
```{r env_model}
clim_mod <- readRDS("saved models/pearson models/clim2.rds")
print(clim_mod$formula)

data <- clim_mod %>% 
  spread_draws(b_total_ppt_mm_cv, `r_Site.x`[site,]) %>% 
  mutate(mean=b_total_ppt_mm_cv + `r_Site.x`) %>% 
...
ggplot(data, aes(x=mean, y=interaction(hilo, Region), color=Region)) +
  ...
  theme_test() +
  scale_color_viridis(option='plasma', discrete = T) +
  ggtitle("Regional response to precipitation variation") +
  ylab("Site") + xlab("Estimate")

scaling <- data.frame(total_ppt_mm_cv=quantile(precip_summary$total_ppt_mm_cv, probs=c(.025, .25, .5, .75, .975)),
                      ppt_cv_raw=quantile(precip_summary_raw$total_ppt_mm_cv, probs=c(.025, .25, .5, .75, .975)))
  
clim_summ <- expand_grid(Site.x=unique(summary_t$Site.x), 
                         scaling) %>% 
  add_fitted_draws(clim_mod, re_formula = ~(total_ppt_mm_cv | Site.x) ) %>% 
  median_qi() %>% 
  left_join(region, by=c("Site.x"="Site"))

env_res <- ggplot(clim_summ, aes(ppt_cv_raw, .value)) +
  ...
  xlab("CV of precipitation for 30 year intervals") +
...
env2 <- precip_b +
  scale_color_viridis(option='plasma', discrete = T) +
  ylab("CV of total annual precipitation") +
  theme(axis.title.x=element_blank()) +
  ggtitle("") +
  theme(plot.margin = unit(c(0,0.3,0.1,0), "cm"))
...
#                          total_ppt_mm_cv=quantile(summary_t$total_ppt_mm_cv, probs=c(.5))) %>% 
#   add_fitted_draws(clim_mod, re_formula = ~(1 | Site.x:pair), allow_new_levels=TRUE) %>% 
#   median_qi(.width=c(0.5,0.75, 0.95)) %>% 
#   group_by(pair) %>% 
#   mutate(.value_avg=median(.value),
#          .lower_avg=median(.lower),
#          .upper_avg=median(.upper)) %>% 
#   left_join(region, by=c("Site.x"="Site"))
# 
# ggplot(clim_summ, aes(pair, .value)) +
#   geom_pointinterval(aes(ymin=.lower, ymax=.upper, color=Site.x), position = position_dodge(width=0.5), alpha=0.5) +
#   # geom_pointinterval(aes(y=.value_avg, ymin=.lower_avg, ymax=.upper_avg)) +
#   ylab("expected correlation") + 
#   labs(caption="Fig. There appears to be lwooittle effect of species pair on the relationship between precip variability and synchrony. ") +
#   theme(plot.caption = element_text(hjust = 0)) +
#   theme_test() +
#   scale_color_viridis(option='plasma', discrete = T)

# clim_summ <- expand_grid(newdata,
#                          total_ppt_mm_cv=quantile(summary_t$total_ppt_mm_cv, probs=c(.025, .25, .5, .75, .975))) %>% 
#   add_fitted_draws(clim_mod, re_formula = NULL, allow_new_levels=TRUE) %>% 
#   median_qi() %>% 
#   left_join(region, by=c("Site.x"="Site"))
# 
# ggplot(clim_summ, aes(total_ppt_mm_cv, .value)) +
#   geom_ribbon(aes(ymin=.lower, ymax=.upper, fill=Region, group=interaction(Region, hilo)), alpha=0.2) +
#   geom_line(aes(color=Region, group=interaction(Region, hilo))) + theme_test() + facet_wrap(~pair)  +
#   ylab("expected correlation") +
#   labs(caption="Fig. Specific expected synchrony response to mean precip variability") +
#   theme(plot.caption = element_text(hjust = 0)) +
#   theme_test() +
#   scale_color_viridis(option='plasma', discrete = T) +
#   scale_fill_viridis(option='plasma', discrete = T)
