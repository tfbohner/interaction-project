...
acgrid <- expand_grid(acdat$.variable[which(acdat$comp=="intra")], acdat$.variable[which(acdat$comp=="inter")]) %>% 
  ...
  inner_join(dplyr::select(acdat, c("var1"=".variable", ".value", "Site", "Neighborhood", "pair"))) %>% 
  inner_join(dplyr::select(acdat, c(".variable", ".value", "Site", "Neighborhood", "pair")), by=c("var2"=".variable"))%>% 
  filter(Site.x==Site.y, Neighborhood.x==Neighborhood.y) %>% 
  mutate(diff=.value.x-.value.y,
         sign=ifelse(diff>0, "pos", "neg"))
length(acgrid$sign[which(acgrid$sign=="pos")])/nrow(acgrid)
...
pjgrid <- expand_grid(pjdat$.variable[which(pjdat$comp=="intra")], pjdat$.variable[which(pjdat$comp=="inter")]) %>% 
  rename(var1=1, var2=2) %>% 
  inner_join(dplyr::select(pjdat, c("var1"=".variable", ".value", "Site", "Neighborhood", "pair"))) %>% 
  inner_join(dplyr::select(pjdat, c(".variable", ".value", "Site", "Neighborhood", "pair")), by=c("var2"=".variable")) %>% 
  filter(Site.x==Site.y, Neighborhood.x==Neighborhood.y) %>% 
  mutate(diff=.value.x-.value.y,
         sign=ifelse(diff>0, "pos", "neg"))
length(pjgrid$sign[which(pjgrid$sign=="pos")])/nrow(pjgrid)
...
plgrid <- expand_grid(pldat$.variable[which(pldat$comp=="intra")], pldat$.variable[which(pldat$comp=="inter")]) %>% 
  rename(var1=1, var2=2) %>% 
  inner_join(dplyr::select(pldat, c("var1"=".variable", ".value", "Site", "Neighborhood", "pair"))) %>% 
  inner_join(dplyr::select(pldat, c(".variable", ".value", "Site", "Neighborhood", "pair")), by=c("var2"=".variable")) %>% 
  filter(Site.x==Site.y, Neighborhood.x==Neighborhood.y) %>% 
  mutate(diff=.value.x-.value.y,
         sign=ifelse(diff>0, "pos", "neg"))
length(plgrid$sign[which(plgrid$sign=="pos")])/nrow(plgrid)
...
ppgrid <- expand_grid(ppdat$.variable[which(ppdat$comp=="intra")], ppdat$.variable[which(ppdat$comp=="inter")]) %>% 
  rename(var1=1, var2=2) %>% 
  inner_join(dplyr::select(ppdat, c("var1"=".variable", ".value", "Site", "Neighborhood", "pair"))) %>% 
  inner_join(dplyr::select(ppdat, c(".variable", ".value", "Site", "Neighborhood", "pair")), by=c("var2"=".variable")) %>% 
  filter(Site.x==Site.y, Neighborhood.x==Neighborhood.y) %>% 
  mutate(diff=.value.x-.value.y,
         sign=ifelse(diff>0, "pos", "neg"))
length(ppgrid$sign[which(ppgrid$sign=="pos")])/nrow(ppgrid)
