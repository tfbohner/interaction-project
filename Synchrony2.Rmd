...
mod12 %>% 
  ...
  labs(caption="Fig. Contrast in intra versus interspecific synchrony effect. Positive contrasts indicate that intraspecific synchrony is greater than interspecific. White fir and sugar pine show synchrony differentiation but not the yellow pines") +
  theme(plot.caption = element_text(hjust = 0))
...
mod12 %>% 
  ...
  geom_vline(xintercept=0, linetype="dashed") +
  theme_test() + ylab("Species pair comparisons") + xlab("Synchrony contrast estimate") +
  labs(caption="Fig. Contrast in intra versus interspecific synchrony effect including region and elevation variation. Positive contrasts indicate that intraspecific synchrony is greater than interspecific. Species level patterns in differentiation predominate although there is noteworthy variation among sites.") +
  theme(plot.caption = element_text(hjust = 0))
...
We calculated pearson correlations between pairs of individuals within neighborhoods for 30 year time blocks with a 10 year lag to measure synchrony through time. Trends were analyzed for data summarized for species-pairs at the neighborhood level. 
...
ggplot(clim_summ, aes(spei12_mean, .value)) +
  ...
  labs(caption="Fig. Regional relationship between SPEI mean and expected synchrony. In the lowest latitudes synchrony is highest in dry periods while in the most mesic site synchrony is higher in the wet periods.") +
  theme(plot.caption = element_text(hjust = 0))
...
ggplot(clim_summ, aes(spei12_sd, .value)) +
  ...
  geom_line(aes(color=Region , group=interaction(Region, hilo))) + theme_test() +
  ylab("expected correlation") + 
  labs(caption="Fig. Regional relationship between SPEI SD and expected synchrony. In the lowest latitudes synchrony is highest in variable periods while in the most mesic site synchrony is higher in the more variable periods") +
  theme(plot.caption = element_text(hjust = 0))
...
# 
# ggplot(clim_summ, aes(x=.value, y=pair, color=Region, group=interaction(Region, hilo))) +
#   geom_pointintervalh(position = position_dodgev(height=0.5)) + theme_test() + 
#   geom_vline(xintercept = 0, linetype="dashed")  +
#   xlab("expected correlation") 
# 
# ggplot(clim_summ, aes(x=.value, y=interaction(hilo, Region), color=pair)) +
#   geom_pointintervalh(position = position_dodgev(height=0.5)) + theme_test() +
#   geom_vline(xintercept = 0, linetype="dashed")  +
#   xlab("expected correlation") 
